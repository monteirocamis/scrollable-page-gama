...
import type { IUniform, TextureDataType, TextureFilter, Vector3Tuple, Vector4Tuple, WebGLRenderTargetOptions } from 'three';
...
export type TAssetTypes = 'model' | 'texture' | 'light' | 'material';
...
export type TSceneEvents = 'sceneUpdate' | 'addSceneObject' | 'update' | 'textureAdded' | 'activeCameraChange' | 'textureUpdate' | 'activeCameraUpdate' | 'sceneMaterialUpdate' | 'environmentChanged' | 'select' | 'objectUpdate' | 'materialUpdate' | 'materialChanged' | 'setView' | 'activateMain' | 'backgroundChanged';
...
export interface IViewerApp extends IEventDispatcher<'update' | 'preRender' | 'postRender' | 'preFrame' | 'postFrame' | 'dispose' | 'addPlugin' | 'renderEnabled' | 'renderDisabled'>, IJSONSerializable {
    state: ViewerState;
    renderer: IRenderer;
    scene: IScene;
...
export interface IViewerPlugin<TViewer extends IViewerApp = IViewerApp, IsSync extends boolean = boolean> extends IEventDispatcher<string>, IUiConfigContainer, Partial<IJSONSerializable> {
    ...
    onDispose?(viewer: TViewer): IsSync extends false ? Promise<void> : void;
...
export interface IViewerPluginSync<TViewer extends IViewerApp = IViewerApp> extends IViewerPlugin<TViewer, true> {
    onAdded(viewer: TViewer): void;
    onRemove(viewer: TViewer): void;
    onDispose?(viewer: TViewer): void;
    dependencies?: Class<IViewerPluginSync>[];
}
export interface IViewerPluginAsync<TViewer extends IViewerApp = IViewerApp> extends IViewerPlugin<TViewer, false> {
